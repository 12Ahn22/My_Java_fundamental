...
public class ReservationUI {
    ...
    public ReservationUI(ReservationManager reservationManager) {
...
//              예매 취소하기
    ...
    private void reserveSeat() {
        ...
        if (reservationManager.isSeatAvailable(rowIndex, colIndex)) {
            System.out.println("예매가 가능합니다. 예매하시겠습니까?");
            System.out.println("네(1), 아니오(2), 초기화면(0)중 하나를 입력해주세요");
            switch (sc.next()) {
                case "1":
                    // 예매 하기
                    int key = reservationManager.reservate(rowIndex , colIndex );
                    System.out.println("예매가 완료되었습니다.");
                    System.out.println("예매한 좌석 번호:" + reservationManager.getReservedSeat(key) + "/ 예매번호:[" + key + "]");
                    break;
                case "2":
                default:
                    break;
    ...
    private void viewReservation() {
        ...
        if (reservationManager.isReserved(key)) {
    ...
    public void printSeatStatus() {
        ...
        for (int i = 0; i < reservationManager.getRow(); i++) {
            for (int j = 0; j < reservationManager.getCol(); j++) {
                if (reservationManager.isSeatAvailable(i, j)) {
    ...
    public void printReservedSeat(int key) {
        System.out.println("고객님이 예매하신 좌석은 " + reservationManager.getReservedSeat(key) + "입니다.");
    }
